...
Let's try modelling a simple GAMMs model based on our data set. 

<!-- In modelling, let's take a similar approach to the static analysis. That is, we will fit a separate model for each combination of ```liquid``` and ```context``` in order to simplify the model interpretation. We'll first subset the data and then convert the variables into factor, a similar procedure to the static analysis.  -->
...
<!-- Let's first just try modelling ```f2z``` for English /l/ in the linear model notation that we are familiar with. That is, we will model the ```f2z``` values as a function of (1) language and (2) vowel context.  -->

<!-- ```{r} -->
<!-- # first model -- linear model -->
<!-- m1 <- mgcv::bam(f2z ~ language + context, data = df_dyn_L, method = "fREML") -->

<!-- # model summary -->
<!-- summary(m1) -->
<!-- ``` -->

<!-- The output looks pretty much the same with the linear mixed-effect modelling using ```lme4::lmer()```.  -->
