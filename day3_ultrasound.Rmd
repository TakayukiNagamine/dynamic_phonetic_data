...
Finally, here's the visualiastion of the joint FPC scores. The data points look quite sparse but this is because FPCA converts each entire trajectory into one number. 
...
m4 <- mgcv::bam(PC1z ~ s(prop_time) + s(prop_time, by = LangSeg.ord) + s(prop_time, speaker, bs = "fs", m = 1), data = dat_sub, method = "ML", rho = rho_m1, AR.start = dat_sub$start.event)
